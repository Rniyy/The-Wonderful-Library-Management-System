...
function migrateFromJsonIfPresent() {
  ...
  const insertCopy = db.prepare(
    'INSERT INTO copies (book_id, copy_number, is_issued, due_date, customer_id) VALUES (?, ?, ?, ?, ?)'
  );
  ...
  try {
    for (const b of oldBooks) {
      ...
      for (const c of copies) {
        insertCopy.run(b.id, c.copyId, c.isIssued ? 1 : 0, c.dueDate || null, c.customerId ?? null);
      }
